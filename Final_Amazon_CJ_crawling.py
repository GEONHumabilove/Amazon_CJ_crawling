...
brands = ['CJ', 'CJ Foods', 'Bibigo', 'Yasik', 'Haechandle', 'cj', 'bibigo', 'yasik', 'haechandle', 'Bulk', 'Cheiljedang', 'cheiljedang']

# 각 페이지에 있는 모든 상품 중에서 CJ 관련 이름이 들어간 상품의 '상품명, 가격, 평점, 상품링크' 추출 후 딕셔너리 형태로 반환하는 get_info() 함수 정의
def get_info(url):
    matching_list = []
    driver.get(url)
...
def get_details():
    ...
    # ingredients, about this item 옆 토글 버튼 클릭해서 확장
    try:
    ...
    try:
        diet_type_list = []
        diet_type_tags = driver.find_elements(By.XPATH, '//span[@class="a-size-base a-color-base _cse-nutritional-info-and-ingredient-card_style_dilText__1iIqY"]')
...
def get_reviews():
    ...
    # 총 리뷰 횟수(ex. 7034 global ratings)
    try:
    ...
    if total_ratings != "None":
        ...
        # By Feature (대표 특징별 평점)
        try:
            features_container = review_container.find_element(By.XPATH, './/div[@id="cr-summarization-attributes-list"]')
            try:
        ...
        # Read Reviews That Mention (키워드 태그들)

        try:
            tag = review_container.find_element(By.XPATH, '//span[@data-cr-trigger-on-view]')
            lighthouse_terms = json.loads(tag.get_attribute('data-cr-trigger-on-view'))['ajaxParamsMap']['lighthouseTerms']
        ...
        # Top Reviews From the United States (고객 리뷰 - 문장)
        ...
        try:
            ...
            for card in cards:
                ...
                star_rate = card.find_element(By.XPATH, './/a[@class="a-link-normal"]').get_attribute("title").split()[0]
        ...
        try:
            see_all_reviews_link = review_container.find_element(By.XPATH, './/a[@data-hook="see-all-reviews-link-foot"]').click()
            time.sleep(2)
            positive_container = driver.find_element(By.CSS_SELECTOR, '.a-column.a-span6.view-point-review.positive-review')
            critical_container = driver.find_element(By.CSS_SELECTOR, '.a-column.a-span6.view-point-review.critical-review')
            positive_review = positive_container.find_element(By.CSS_SELECTOR, 'div.a-row.a-spacing-top-mini > span').text
            critical_review = critical_container.find_element(By.CSS_SELECTOR, 'div.a-row.a-spacing-top-mini > span').text
...
for i in range(len(links)):
    ...
    # page 안에는 상품의 정보를 담은 딕셔너리들이 들어있음
    for dictionary in page:
        url = dictionary['link']
        # 해당 상품의 브랜드 값 비교 후 brand가 CJ이면 product_list_CJ에 저장
        driver.get(url)
        try:
